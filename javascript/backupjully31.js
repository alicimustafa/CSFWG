		...
		if(sec.classList.contains("open-section")){
			...
			xhr.onreadystatechange = function() {
				if(xhr.readyState == 4 && xhr.status == 200){
					document.getElementById("calendar-pannel").innerHTML = xhr.responseText;
					sec.classList.add("open-section");
					sec.classList.remove("closed-section");
					calendar_add_events();
				}
			}
			xhr.open("GET", url_link+"calendar" , true);
			xhr.send();
		}
	}
	function calendar_add_events(){
		/*
		this function adds event listeners to the calendar tab after calendar loads from the server
		*/
		document.getElementById("left-arrow").addEventListener('click', change_month, false);
		document.getElementById("right-arrow").addEventListener('click', change_month, false);
	}
	function change_month(ev){
		...
		xhr.open("GET", url_link+"calendar/changeMonth/"+link_year+"/"+link_month, true);
		xhr.send();
	}
    ...
    return {
		...
		*/
		start: function(){
			...
			document.getElementById("calendar-icon").addEventListener('click', calendar_open, false);
			var url_info = location.search;
			if(url_info == ""){
